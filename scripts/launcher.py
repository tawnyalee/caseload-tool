...
class App:
    ...
    def _popout_data_panel(self) -> None:
        """Pop the Data panel into its own window (or raise it if already
        popped). Triggered from the always-visible log header, so the data
        views/charts are reachable even when the docked Data tab is squeezed."""
        dp = getattr(self, "data_panel", None)
        if dp is None:
            return
        if dp.popped and dp.window is not None:
            try:
                dp.window.deiconify()
                dp.window.lift()
                dp.window.focus_force()
            except Exception:
                pass
        else:
            dp._pop_out()
